...
## Rule-Induction: Hypothesen HYP1–HYP7 (räumliche Einbettung, Stand 2026-09-05)
...
- **HYP4 „H1-Zonen-Revisit"** (Cross-Timeframe-Hypothese, TF=H1↔M3): wird ein H1-DREIER erneut angelaufen (`IS_TOUCHED` bei `status OPEN`), sind die M3-Patterns relevant, die räumlich innerhalb der H1-Preiszone UND zeitlich während der H1-Musterbildung (`[H1.startTime, H1.validFrom]`) entstanden sind — gestaffelt StartOfMove > Postbreak-Retest-Trendbruch > offenes Pattern in der Zone. Für `IS_POSTBREAK_RETEST_TOUCHED` (H1-Pattern bereits `BROKEN`) gilt eine Sonderregel mit drei Marken um die Bruchkante: (1) Touch des höchsten M3-Gegenpatterns der bruchauslösenden Bewegung, (2) Bruch des letzten offenen M3-Patterns oberhalb der Bruchkante, (3) Retest der Bruchkante selbst.

**Neu, Stand 2026-09-05, noch nicht getestet — konzeptionell erfasst, keine Code-/Datenanalyse dazu:**
- **HYP5 „H1-Zonen-Break"** (Cross-Timeframe-Hypothese, TF=H1↔M3): wird ein H1-DREIER gebrochen, sind die M3-Patterns relevant, die den Impuls zum Break gegeben haben.
- **HYP6 „Trendbruch vor Erreichen des Ziels"** (Cross-Timeframe-Hypothese, TF=H1↔M3): ein H1-DREIER wurde gebrochen, der Kurs bestätigt zunächst mit einem M3-DREIER den H1-Bruch, fällt dann aber so, dass der M3-Patterntrend gebrochen wird. Einstieg: STOP-Order am Bruch oder LIMIT an der darüberliegenden M3-TANGENTE.
- **HYP7 „M3/M1-Hammer"**: in einer Situation mit starker Spannung, kurz unter oder über einem offensichtlichen Ziel — STOP-Order unter/über M3/M1-HAMMER.
...
- TANGENTEN-Zusatzfelder in `relations` (`tangenteIndex`, `tangAngle`, `tangLength`) bewusst zurückgestellt, eigenes Thema: `tangenteIndex` ist hart auf `3` gesetzt (Feld-Kommentar sagt "1 oder 2", kein echtes FIFO-Slot-Tracking vorhanden), `tangAngle`/`tangLength` sind nie implementiert (nur auskommentiert). `eventSlopeRelation`/`tangSlope` wurden im Zuge der Feldprüfung bereits gefixt (siehe Commit-Historie) und sind an Live-Daten verifiziert.
- `eventTime` ist nur *pro Pattern* garantiert monoton zur `eventId` (verifiziert: 0 Verstöße über alle Patterns), NICHT über die globale `eventId`-Reihenfolge hinweg. Beispiel aus den frischen Daten: eventId 864 (M3-DREIER-IS_BROKEN, eventTime 2026-08-04 00:15) wird unmittelbar gefolgt von eventId 865 (H1-VTH-IS_CREATED, eventTime 2026-08-03 19:00 — 5h15min früher). Ursache vermutlich: VTH-`IS_CREATED` wird mit der Bar-Zeit der ursprünglichen Formation gestempelt (`bb.bars[1].time`), die eigentliche Erkennung/Bestätigung im Code passiert aber erst deutlich später (VTH hat ohnehin die bekannte `validFrom = dayBar0*86400`-Sonderkonvention, siehe oben). Für Rule-Induction-Queries, die sich pro Event/Pattern bewegen (nicht über die rohe `eventId`-Sequenz), unkritisch — aber im Hinterkopf behalten, falls mal event-übergreifend über die globale Reihenfolge gearbeitet wird.
- **LIMIT-Fill-Simulation verlangt einen zusätzlichen Bestätigungstick nach dem Touch, was im echten Handel so nicht nötig wäre** (`TradeManager.mqh::OnTick_Waiting()`): `entryPrice = eventPrice + delta*direction` liegt für LONG 1 Punkt ÜBER dem Touch-Preis; die Order wird aber erst beim `IS_TOUCHED`-Event angelegt (Status `TRADE_WAITING`) und füllt erst, wenn ein SPÄTERER Tick `t.last >= entryPrice` erreicht — nie auf demselben Tick wie der Touch selbst. Führt zu `EXIT_NOFILL`-Fällen (bei Session-Ende noch `TRADE_WAITING`, aktuell 26 von 678 in einer Stichprobe, ~4%), obwohl im echten Handel die Limit-Order schon bei Annäherung an den Level (Preis+1) im Markt läge und potenziell sofort/auf demselben Tick füllen würde. Zurückgestellt, nicht angefasst — 2026-09-03 aufgefallen bei der HYP4-Formationsfenster-Auswertung.
