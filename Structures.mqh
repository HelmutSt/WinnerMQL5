...
string sql_create_view =
   "DROP VIEW IF EXISTS rule_induction_matrix;\n"
   "CREATE VIEW rule_induction_matrix AS\n"
   "SELECT  "
   "    v.slPoints              AS variant_stopLoss,\n"
   "    v.tpPoints              AS variant_takeProfit,\n"
   "    t.exitPrice             AS result_exitPrice,\n"
   "    t.exitTime              AS result_exitTime,\n"
   "    t.exitReason            AS result_exitReason,\n"
   "    t.profit                AS result_profit,\n"
   "    t.mae                   AS result_mae,\n"
   "    t.mfe                   AS result_mfe,\n"
   "    e.eventId               AS event_id,\n"
   "    e.eventTime             AS event_time,\n"
   "    e.eventPrice            AS event_price,\n"
   "    e.eventReason           AS event_reason,\n"
   "    e.eventDirection        AS event_direction,\n"
   "    e.patternId             AS event_patternId,\n"
   "    m.spread                AS market_spread,\n"
   "    m.atrM3                 AS market_atr,\n"
   "    m.atrRatioM3_H1         AS market_volatility,\n"
   "    m.relativeVolume        AS market_relativeVolume,\n"
   "    m.session               AS market_session,\n"
   "    m.sessionPhase          AS market_sessionPhase,\n"
   "    m.isNews                AS market_isNews,\n"
   "    m.newsImpact            AS market_newsImpact,\n"
   "    m.newsType              AS market_newsType,\n"
   "    r.slotName              AS rel_slotName,\n"
   "    r.patternId             AS rel_patternId,\n"
   "    r.priceDistance         AS rel_priceDistance,\n"
   "    r.barsSinceStart        AS rel_barsSinceStart,\n"
   "    r.barsSinceBreak        AS rel_barsSinceBreak,\n"
   "    r.priceDistanceToBreakLevel AS rel_distBreakLevel,\n"
   "    r.priceDistanceExtremeBeforeBreak AS rel_extBeforeBreak,\n"
   "    r.priceDistanceExtremeAfterBreak  AS rel_extAfterBreak,\n"
   "    r.eventSlopeRelation    AS rel_slopeRelation,\n"
   "    r.eventPositionRelative AS rel_positionRelative,\n"
   "    r.isOriginPattern       AS rel_isOriginPattern,\n"
   "    r.isInPatternNow        AS rel_isInPatternNow,\n"
   "    r.isTouchingNow         AS rel_isTouchingNow,\n"
   "    r.isNearTouchingNow     AS rel_isNearTouchingNow,\n"
   "    r.isBreakingNow         AS rel_isBreakingNow,\n"
   "    r.patternRelevanceAtEvent AS rel_relevance,\n"
   "    pc.type                 AS rel_pattern_type,\n"
   "    pc.timeFrame            AS rel_pattern_tf,\n"
   "    pc.priceLow             AS rel_pattern_low,\n"
   "    pc.priceHigh            AS rel_pattern_high,\n"
   "    pc.priceSlopePerBar     AS rel_pattern_slope,\n"
   "    pc.patternStrengthAtCreate AS rel_pattern_strength,\n"
   "    pc.startBarIndex        AS rel_pattern_startBar,\n"
   "    pd.status               AS rel_pattern_status,\n"
   "    pd.breakBarIndex        AS rel_pattern_breakBar,\n"
   "    pd.priceExtremeBeforeBreak AS rel_pattern_extBefore,\n"
   "    pd.priceExtremeAfterBreak  AS rel_pattern_extAfter,\n"
   "    pc_event.type           AS event_pattern_type,\n"
   "    pc_event.timeFrame      AS event_pattern_tf,\n"
   "    pc_event.priceLow       AS event_pattern_low,\n"
   "    pc_event.priceHigh      AS event_pattern_high,\n"
   "    pc_event.priceSlopePerBar AS event_pattern_slope,\n"
   "    pc_event.patternStrengthAtCreate AS event_pattern_strength,\n"
   "    pc_event.startBarIndex  AS event_pattern_startBar,\n"
   "    pd_event.status         AS event_pattern_status,\n"
   "    pd_event.breakBarIndex  AS event_pattern_breakBar,\n"
   "    pd_event.priceExtremeBeforeBreak AS event_pattern_extBefore,\n"
   "    pd_event.priceExtremeAfterBreak  AS event_pattern_extAfter,\n"
   "    e1.eventPrice           AS story1_price,\n"
   "    e1.eventTime            AS story1_time,\n"
   "    pc1.type                AS story1_type,\n"
   "    pd1.status              AS story1_status,\n"
   "    e2.eventPrice           AS story2_price,\n"
   "    e2.eventTime            AS story2_time,\n"
   "    pc2.type                AS story2_type,\n"
   "    pd2.status              AS story2_status,\n"
   "    e3.eventPrice           AS story3_price,\n"
   "    e3.eventTime            AS story3_time,\n"
   "    pc3.type                AS story3_type,\n"
   "    pd3.status              AS story3_status,\n"
   "    e4.eventPrice           AS story4_price,\n"
   "    e4.eventTime            AS story4_time,\n"
   "    pc4.type                AS story4_type,\n"
   "    pd4.status              AS story4_status,\n"
   "    e5.eventPrice           AS story5_price,\n"
   "    e5.eventTime            AS story5_time,\n"
   "    pc5.type                AS story5_type,\n"
   "    pd5.status              AS story5_status,\n"
   "    e6.eventPrice           AS story6_price,\n"
   "    e6.eventTime            AS story6_time,\n"
   "    pc6.type                AS story6_type,\n"
   "    pd6.status              AS story6_status,\n"
   "    e7.eventPrice           AS story7_price,\n"
   "    e7.eventTime            AS story7_time,\n"
   "    pc7.type                AS story7_type,\n"
   "    pd7.status              AS story7_status,\n"
   "    e8.eventPrice           AS story8_price,\n"
   "    e8.eventTime            AS story8_time,\n"
   "    pc8.type                AS story8_type,\n"
   "    pd8.status              AS story8_status\n"
   "FROM events e\n"
   "LEFT JOIN market m       ON m.eventId = e.eventId\n"
   "LEFT JOIN trades t       ON t.eventId = e.eventId\n"
   "LEFT JOIN variants v     ON v.variantId = t.variantId\n"
   "LEFT JOIN relations r    ON r.eventId = e.eventId\n"
   "LEFT JOIN patternDynamic pd\n"
   "       ON pd.eventId   = r.eventId\n"
   "      AND pd.patternId = r.patternId\n"
   "LEFT JOIN patternCore pc ON pc.patternId = r.patternId\n"
   "LEFT JOIN patternDynamic pd_event\n"
   "       ON pd_event.eventId   = e.eventId\n"
   "      AND pd_event.patternId = e.patternId\n"
   "LEFT JOIN patternCore pc_event ON pc_event.patternId = e.patternId\n"
   "LEFT JOIN events e1  ON e1.eventId  = e.eventId - 1\n"
   "LEFT JOIN events e2  ON e2.eventId  = e.eventId - 2\n"
   "LEFT JOIN events e3  ON e3.eventId  = e.eventId - 3\n"
   "LEFT JOIN events e4  ON e4.eventId  = e.eventId - 4\n"
   "LEFT JOIN events e5  ON e5.eventId  = e.eventId - 5\n"
   "LEFT JOIN events e6  ON e6.eventId  = e.eventId - 6\n"
   "LEFT JOIN events e7  ON e7.eventId  = e.eventId - 7\n"
   "LEFT JOIN events e8  ON e8.eventId  = e.eventId - 8\n"
   "LEFT JOIN patternCore pc1  ON pc1.patternId  = e1.patternId\n"
   "LEFT JOIN patternCore pc2  ON pc2.patternId  = e2.patternId\n"
   "LEFT JOIN patternCore pc3  ON pc3.patternId  = e3.patternId\n"
   "LEFT JOIN patternCore pc4  ON pc4.patternId  = e4.patternId\n"
   "LEFT JOIN patternCore pc5  ON pc5.patternId  = e5.patternId\n"
   "LEFT JOIN patternCore pc6  ON pc6.patternId  = e6.patternId\n"
   "LEFT JOIN patternCore pc7  ON pc7.patternId  = e7.patternId\n"
   "LEFT JOIN patternCore pc8  ON pc8.patternId  = e8.patternId\n"
   "LEFT JOIN patternDynamic pd1\n"
   "       ON pd1.eventId   = e1.eventId\n"
   "      AND pd1.patternId = e1.patternId\n"
   "LEFT JOIN patternDynamic pd2\n"
   "       ON pd2.eventId   = e2.eventId\n"
   "      AND pd2.patternId = e2.patternId\n"
   "LEFT JOIN patternDynamic pd3\n"
   "       ON pd3.eventId   = e3.eventId\n"
   "      AND pd3.patternId = e3.patternId\n"
   "LEFT JOIN patternDynamic pd4\n"
   "       ON pd4.eventId   = e4.eventId\n"
   "      AND pd4.patternId = e4.patternId\n"
   "LEFT JOIN patternDynamic pd5\n"
   "       ON pd5.eventId   = e5.eventId\n"
   "      AND pd5.patternId = e5.patternId\n"
   "LEFT JOIN patternDynamic pd6\n"
   "       ON pd6.eventId   = e6.eventId\n"
   "      AND pd6.patternId = e6.patternId\n"
   "LEFT JOIN patternDynamic pd7\n"
   "       ON pd7.eventId   = e7.eventId\n"
   "      AND pd7.patternId = e7.patternId\n"
   "LEFT JOIN patternDynamic pd8\n"
   "       ON pd8.eventId   = e8.eventId\n"
   "      AND pd8.patternId = e8.patternId;\n";
#endif
